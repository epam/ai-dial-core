...
public class ControllerSelector {
    ...
    private final Pattern PATTERN_POST_DEPLOYMENT = Pattern.compile("/+openai/deployments/([-.@a-zA-Z0-9]+)/(completions|chat/completions|embeddings)");
    private final Pattern PATTERN_DEPLOYMENT = Pattern.compile("/+openai/deployments/([-.@a-zA-Z0-9]+)");
    private final Pattern PATTERN_DEPLOYMENTS = Pattern.compile("/+openai/deployments");

    private final Pattern PATTERN_MODEL = Pattern.compile("/+openai/models/([-.@a-zA-Z0-9]+)");
    private final Pattern PATTERN_MODELS = Pattern.compile("/+openai/models");

    private final Pattern PATTERN_ADDON = Pattern.compile("/+openai/addons/([-.@a-zA-Z0-9]+)");
    private final Pattern PATTERN_ADDONS = Pattern.compile("/+openai/addons");

    private final Pattern PATTERN_ASSISTANT = Pattern.compile("/+openai/assistants/([-.@a-zA-Z0-9]+)");
    private final Pattern PATTERN_ASSISTANTS = Pattern.compile("/+openai/assistants");

    private final Pattern PATTERN_APPLICATION = Pattern.compile("/+openai/applications/([-.@a-zA-Z0-9]+)");
    private final Pattern PATTERN_APPLICATIONS = Pattern.compile("/+openai/applications");
