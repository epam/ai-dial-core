...
| storage.layout.migration.refreshPeriodSeconds | 10 |    No    | How often each node reloads the per-bucket migration state while a store is migrated one bucket at a time. A change reaches every node within twice this period, and a bucket is held against writes for at least that long while it moves — lower it to shorten the pause, raise it to reduce reads of the state document. Unused when `storage.layout.tenantRooted` is enabled, which serves the whole store from one layout. |
