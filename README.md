...
| analytics.collectHeaders                 |       false        |    No    | If `true`, adds a `headers` object with the request headers to each analytics log entry. Headers listed in `analytics.headersBlacklist` are excluded. **Note**: enabling this may write sensitive data to logs; curate the blacklist for your setup.                                                                  |
| analytics.headersBlacklist               | authorization,api-key,cookie,proxy-authorization | No | Comma-separated, case-insensitive list of request header names excluded when `analytics.collectHeaders` is enabled.                                                                                                                                                                           |
