...
| analytics.collectEmail                   |       false        |    No    | If `true`, adds `user_email` (the raw, unhashed email taken from `identityProviders.*.userEmailPath`) to the `claims` object of each analytics log entry. Independent of `analytics.collectClaims`: either flag alone is enough to emit the `claims` object, and each contributes only its own members. The member is omitted when the claim is absent (e.g. API key requests, which carry no JWT claims). **Note**: this writes plain-text personal data to logs. When `identityProviders.*.loggingKey` points at the email claim and `identityProviders.*.obfuscateUserEmail` is left enabled, `user.id` already identifies the same user as a salted hash — prefer that unless you specifically need the raw address.        |
...
| identityProviders.*.userEmailPath        |  email  |    No    | Path to the claim in JWT token or user info response where the user email can be taken. Unlike `loggingKey` the value is never hashed. The claim is extracted on every authenticated request — like the rest of the JWT payload, it travels with the per-request API key data and with background job records — and `analytics.collectEmail` only controls whether it also reaches the analytics log. **Note**: `email` might be unavailable for some IDPs; check your IDP documentation and point this at the right claim (e.g. `preferred_username` or `upn`).                                                                                                    |
