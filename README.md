...
| config.migrateLegacyEndpoints            |       false        |    No    | When `true`, DIAL Core rewrites config files that still use `endpoint`/`responsesEndpoint` to the `interfaces` shape on disk (in-memory migration always happens regardless). Settable via the `aidial.config.migrateLegacyEndpoints` environment variable. See [Migrating `endpoint`/`responsesEndpoint` to `interfaces`](#migrating-endpointresponsesendpoint-to-interfaces). |
...
#### LLM API interfaces (`interfaces`)

Each [model](/docs/dynamic-settings/models.md), [application](/docs/dynamic-settings/applications.md) and [interceptor](/docs/dynamic-settings/interceptors.md) declares the set of LLM API interfaces it exposes via the typed `interfaces` map. Each entry maps an interface type to a `base_url` pointing at the adapter/service root; at request time DIAL Core forwards to `{base_url}` + the exact ingress path it received (e.g. `POST /openai/v1/responses` is proxied to `{base_url}/openai/v1/responses`).

The supported LLM API interface types are:

| Interface type          | Routed ingress path(s)                                                              | Description                                                                              |
|-------------------------|-------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| `openaiChatCompletions` | `/openai/deployments/{id}/{chat/completions\|completions\|embeddings}`              | The OpenAI "deployments" POST family — chat completions, completions and embeddings.     |
| `openaiResponses`       | `/openai/v1/responses`, `/openai/v1/responses/{id}`, `/openai/v1/responses/{id}/cancel` | The OpenAI Responses API.                                                            |

```json
"interfaces": {
  "openaiChatCompletions": { "base_url": "http://adapter:5000" },
  "openaiResponses":       { "base_url": "http://adapter:5000" }
}
```

#### Migrating `endpoint`/`responsesEndpoint` to `interfaces`

The deployment-level `endpoint` and `responsesEndpoint` fields are **deprecated** in favor of the typed `interfaces` map above. DIAL Core always migrates the legacy fields **in memory** on every config load — the authority (`scheme://host[:port]`) of the old endpoint becomes the interface `base_url` (`endpoint` → `openaiChatCompletions`, `responsesEndpoint` → `openaiResponses`), and the ingress path is appended at request time — so routing keeps working with no config change required. Interceptors are routed exactly like models and applications.

To additionally rewrite the config **files on disk** to the `interfaces` shape, enable the `config.migrateLegacyEndpoints` [static setting](#static-settings) — for example via the environment variable `aidial.config.migrateLegacyEndpoints=true` (static settings are read from environment variables prefixed with `aidial.`). When enabled, on every config load DIAL Core migrates each `models`/`applications`/`interceptors` entry that still uses `endpoint`/`responsesEndpoint` and atomically writes the file back in the `interfaces` shape, preserving all other fields and key order. Read-only or non-regular config sources (e.g. a Kubernetes ConfigMap or a classpath resource) are skipped and migrated in memory only. When disabled (the default) files are left untouched and DIAL Core logs a warning for any entry still using the legacy fields.
