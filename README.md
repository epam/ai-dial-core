...
| access.readonlyAdmin.rules     |    -    |    No    | Matches claims from identity providers with the rules to figure out whether a user is allowed to perform read-only admin actions (READ access to all public and private resources). Intended for background or scheduled jobs that need to read user data without write privileges. Not active by default — must be explicitly configured. Applications (per-request API keys) are excluded even if they carry a matching role. <br /> **Configuration example**:<br /> "access": {"readonlyAdmin": {"rules": [{"function": "EQUAL","source": "roles","targets": ["readonly-admin"]}]}} |
