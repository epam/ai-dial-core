...
| analytics.collectErrorResponses          |       false        |    No    | If `true`, also writes an analytics log entry for a `429`/`502`/`503`/`504` response that reaches the client only after a rate limit hit, all upstreams failing/being retried, or a connection failure — none of which get logged otherwise. Does not affect a deployment/upstream response that already carries one of these statuses through as its own answer (that is logged either way, like any other status). |
