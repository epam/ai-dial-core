...
| log.collectClaims                        |       false        |    No    | If `true`, adds a `claims` object (`user_id`, `roles`, `project`, `user_hash`, `user_display_name`) to each analytics log entry. **Note**: these claims may contain personal data (e.g. user id or display name); enable only where logging such data is acceptable.                                              |
| log.collectHeaders                       |       false        |    No    | If `true`, adds a `headers` object with the request headers to each analytics log entry. Headers listed in `log.headersBlacklist` are excluded. **Note**: enabling this may write sensitive data to logs; curate the blacklist for your setup.                                                                  |
| log.headersBlacklist                     | authorization,api-key,cookie,proxy-authorization | No | Comma-separated, case-insensitive list of request header names excluded when `log.collectHeaders` is enabled.                                                                                                                                                                           |
