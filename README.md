...
| identityProviders.*.userEmailPath        |  email  |    No    | Path to the claim in JWT token or user info response where the user email can be taken. Used only when `analytics.collectEmail` is enabled, and unlike `loggingKey` the value is never hashed. **Note**: `email` might be unavailable for some IDPs; check your IDP documentation and point this at the right claim (e.g. `preferred_username` or `upn`).                                                                                                    |
