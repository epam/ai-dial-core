...
| storage.layout.tenantRooted  |   false    |    No    | Places every bucket under a tenant root instead of storing it at the top level. Serves the whole store from that layout, so it is for an empty store or one whose migration has completed; the server refuses to start on a store that holds legacy data no migration has moved, or whose migration has not been declared complete. |
...
| storage.layout.migration.enabled |   false    |    No    | Enables migrating a populated store to the tenant-rooted layout one bucket at a time. While disabled the per-bucket state is neither read nor polled, and every bucket is served from the legacy layout. Turn it on before migrating anything; a rolling restart is safe, since a store with nothing migrated resolves the same layout either way. The environment is closed to new resources for the duration of a migration: a bucket created while its parent has already moved would be split across the two layouts. Once every bucket has moved, the migration is declared complete, after which new buckets resolve to the tenant-rooted layout and `storage.layout.tenantRooted` can be enabled in its place. Unused when `storage.layout.tenantRooted` is enabled, which serves the whole store from one layout. |
| storage.layout.migration.refreshPeriodSeconds | 10 |    No    | How often each node reloads the per-bucket migration state. A change reaches every node within twice this period, and a bucket is held against writes for at least that long while it moves — lower it to shorten the pause, raise it to reduce reads of the state document. The bound is why this is not slowed down when no migration is running; `enabled` decides that instead. |
