...
| analytics.claimsAllowlist                |     _(empty)_      |    No    | JSON array of claims to add to the `claims` object of each analytics log entry, taken verbatim from the identity provider's token or user info response. Each entry is a dot-separated path (e.g. `email`, `resource_access.roles`) and is emitted under that exact path as its key, so a single list covers several providers that name the same thing differently (`email`, `upn`, `preferred_username`) — missing claims are simply omitted. `["*"]` collects every top-level claim. Independent of `analytics.collectClaims`: either setting alone is enough to emit the `claims` object, and each contributes only its own members; entries colliding with `user_id`, `roles` or `user_display_name` are skipped while `collectClaims` is on. **Note**: claim values are written unhashed, so this puts plain-text personal data (email, name) in logs. When `identityProviders.*.loggingKey` points at the email claim and `identityProviders.*.obfuscateUserEmail` is left enabled, `user.id` already identifies the same user as a salted hash — prefer that unless you specifically need the raw values.        |
