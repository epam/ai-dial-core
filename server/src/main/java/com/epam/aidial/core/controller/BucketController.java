...
public class BucketController {
    ...
    public Future<?> getBucket() {
        ...
        String bucketLocation = ProxyUtil.buildUserBucket(context);
        String encryptedBucket = encryptionService.encrypt(bucketLocation);
        String appDataBucket = ProxyUtil.buildAppDataBucket(context);
        ...
        if (appDataBucket == null) {
            ...
            appDataLocation = encryptedAppDataBucket + BlobStorageUtil.PATH_SEPARATOR + ProxyUtil.APPDATA_PATTERN.formatted(encodedSourceDeployment);
        }
