...
Per-entity GET is **blob-only** — file-sourced entries from `aidial.config.json` are not addressable here. The operator-facing path for file entries is `/v1/admin/config/file/*`, described below under operator endpoints.
...
- **Cluster-wide serialization.** Every admin write surface — bulk apply and per-entity `PUT` / `DELETE` alike — runs under the `public/` + `platform/` bucket locks (`underBucketLocks`; see `02-architecture.md`), which also gate the full-rebuild blob scan. Concurrent admin writes on different pods serialize at the entity-set level; an in-flight apply cannot have another batch's writes interleaved into it, and no peer pod's rebuild can scan partial-batch state.
