...
The singleton `settings update` is the documented exception (the global-settings projection always has a value — `PUT` is upsert by nature).
...
- **Connection** — `api_url`, `auth` per environment.
...
Three substitution namespaces drive template resolution:
...
- **As-is** — no transformation. Right for entity types with no env-specific fields (roles, keys, routes).
