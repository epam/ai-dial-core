...
public class AdminRoleAuthorizationService implements ConfigAuthorizationService {
    public boolean isAuthorized(ProxyContext context, String entityType,
        ...
        if (PUBLIC_BUCKET.equals(bucket)) {
            // Reaching here means the request is already authenticated (Proxy rejects otherwise),
            // so public reads are open; writes require explicit admin (fail-closed, see §1.3).
...
**Fail-closed when `access.admin.rules` is empty or unconfigured.** The legacy `AccessService.hasAdminAccess` delegates to `RuleMatcher`, which treats an empty rule list as **allow-all** — every authenticated caller is admin. That default is acceptable for the pre-existing Resource API surfaces (publications, public-resource browsing) but is the wrong default for the Configuration/Admin API, where it would silently expose platform secrets and config mutation to everyone. The Configuration/Admin API therefore gates on a distinct seam, `AccessService.hasExplicitAdminAccess(context) = adminRulesConfigured && hasAdminAccess(context)`, where `adminRulesConfigured` is `false` when `access.admin.rules` is missing or empty. With an empty/unconfigured rule set this **denies all** Configuration/Admin access: platform reads and writes, public writes, and every `/v1/admin/*` endpoint return `403` — even for a caller who would otherwise satisfy the admin role. Public **reads** stay open (any authenticated caller), since they carry no secrets and follow the existing `public/` rule. The legacy `hasAdminAccess` and all other callers (publication review, `getAdminAccess` permission rule, `filterForbidden`, `hasPublicAccess`) retain the `RuleMatcher` empty→allow-all behavior unchanged — only the `AdminRoleAuthorizationService` call sites move to `hasExplicitAdminAccess`. Operators who rely on the config/admin API must configure at least one admin rule; see [`07-migration-and-rollout.md`](07-migration-and-rollout.md).
...
**`forwardAuthToken` admin-only write carve-out (slice U.3).** The `forwardAuthToken` boolean on `Application` and `ToolSet` (inherited from `Deployment`) is a security-sensitive field — when `true`, the deployment receives the caller's downstream auth token. By default `ApplicationService.putApplication` / `ToolSetService.putToolSet` strip the field to `false` on every write (pre-existing policy). Slice U.3 introduces a single carve-out: **admin writes to `public/`** (via per-entity `PUT /v1/{applications|toolsets}/public/{name}` and via bulk `POST /v1/admin/apply`) may set `forwardAuthToken: true` and the value is persisted. User-bucket writes, non-admin writes to `public/` (rejected at the authz layer), and `PublicationService` re-publishing user content all continue to strip the field — defense in depth so an admin-approved publication of a user-authored asset cannot smuggle in `forwardAuthToken: true` that the user-bucket strip would have caught. The carve-out is detected at the controller layer as `descriptor.isPublic() && AccessService.hasAdminAccess(context)` and passed through the service signature as a `boolean preserveForwardAuthToken` argument; `false` is the default at every other call site.

### 1.5 Response projection: Public / Owner views

**Scope — per-entity GET only.** The Public / Owner projection described in this section applies to per-entity `GET /v1/{type}/{bucket}/{name}` responses. The folder metadata listing surface (`GET /v1/metadata/{type}/{bucket}/{path}`) returns the existing `ResourceItemMetadata` shape (`url`, `name`, `etag`, `createdAt`, `updatedAt`, `author`) and carries no projection — see [`03-api-reference.md`](03-api-reference.md) §4. Operational metadata that admins/owners need on a per-entity GET (`validationWarnings`) must not leak to Public callers; entity properties on the **per-type Public allowlist** (see §1.5 bucket-scope clarification below) and validity status (`status`) are public-safe. Two Jackson views handle this declaratively. *(Slice U.1, 2026-05-21: the previously Owner-only `source: "file" | "api"` field has been retired from the per-entity Configuration API — the URL itself discloses the source. File-sourced entries are no longer addressable on `/v1/{type}/{bucket}/{name}`; operators inspect them via `/v1/admin/config/file/{type}[/{name}]` — see "File-config inspection surface" below.)*
...
| Field | View | Notes |
|---|---|---|
| Allowlisted entity properties on the per-type Public surface (e.g. for `Model`: `id`, `displayName`, `displayVersion`, `iconUrl`, `description`, `features`, `capabilities`, `tokenizerModel`, `limits`, `pricing`, `attachmentTypes`, `defaults`, `owner`, `createdAt`, `updatedAt`) | `Public` | Modeled on the existing `ModelData` / `ApplicationData` DTOs (see §1.5 bucket-scope clarification). Each `public/`-bucket type carries an explicit allowlist; new fields default to invisible (`DEFAULT_VIEW_INCLUSION = false`) until annotated. |
| Infrastructure fields on the entity (e.g. `Model.upstreams`, `Upstream.endpoint`, `Upstream.extraData`, `Model.interceptors`, `Application.endpoint`, schema-by-URI references that disclose internal hostnames) | `Owner` | Never on the Public allowlist. Owner callers (admin or bucket-owner) see them; plain authenticated readers do not. |
...
Authorization (post-U.4):

- **All types except `keys`** — admin role (same gate as the rest of `/v1/admin/*`).
- **`keys`** — denied for every caller, including admin. The file-sourced `Config.keys` map uses the legacy format where the map key *equals the secret value* ([OQ-12](08-open-questions-and-references.md)). Slice U.4 retired the `security-admin` tier that previously gated this carve-out; with no operator role distinguishing "admin" from "may read secret-equivalent names", the surface denies the `keys` type unconditionally. Operators with strict need read `aidial.config.json` directly off the deployment.
...
**Future-field rules.** New Public field → top level on entity, `@JsonView(Public)`. New Owner-only operational metadata → top level on response wrapper, `@JsonView(Owner)`. New Owner-only **state** field that's part of the entity body (e.g., a future `regionOverride` flag the owner sets) → top level on entity, `@JsonView(Owner)`. Container-vs-flat is decided once: flat shape for everything; the view annotation is the gating mechanism. Secrets stay on their own track — `@EncryptedField` + `@JsonProperty(WRITE_ONLY)` drops the field from every response (§2.5). Slice U.4 retired the `?reveal_secrets=true` reveal flow and the `security-admin` tier; the projection model and the secrets-at-rest model compose orthogonally and there is no plaintext-read path on the API.

**Bucket-scope clarification — Public view shape only matters for `public/`-bucket types.** The Public view is observable only for `public/`-bucket types (`models`, `applications`, `toolsets`, `schemas`, `files`, `prompts`, `conversations`). `platform/`-bucket types (`interceptors`, `roles`, `keys`, `routes`, `settings`) are gated to admin-only at the authorization dispatch layer (§1.2) — non-admin callers never reach the projection step. Implementations MUST NOT provide a Public-view serialization code path for `platform/`-bucket types; the controller always emits the Owner view for them.

**Public view is a per-type allowlist, not "everything except secrets".** For each `public/`-bucket entity type, the Public projection exposes a hand-picked allowlist of properties safe for any authenticated reader. The allowlists match today's existing public-projection DTOs:

- `models` → the field set of `ModelData` (`server/src/main/java/com/epam/aidial/core/server/data/ModelData.java`) populated by `ModelController.createModel(...)`: `id`, `displayName`, `displayVersion`, `iconUrl`, `description`, `features`, `capabilities`, `tokenizerModel`, `limits`, `pricing`, `attachmentTypes`, `defaults`, `owner`, `createdAt`, `updatedAt`. **Not** `upstreams`, `Upstream.endpoint`, `Upstream.extraData`, `Upstream.key`, `interceptors`, `overrideName`, `fieldsHashingOrder`.
- `applications` → the field set of `ApplicationData` (`server/src/main/java/com/epam/aidial/core/server/data/ApplicationData.java`) populated by `ApplicationData.mapApplication(...)`: identity, display metadata, `features`, `applicationProperties`, `applicationTypeSchemaId`, `function`, `routes`, `viewerUrl`, `editorUrl`, attachment types, defaults. **Not** `endpoint`, `Application.env` (encrypted operator-managed), interceptor references.
- `toolsets`, `schemas`, admin-managed `files` / `prompts` / `conversations` → analogous per-type allowlists matching the substance each user-facing surface already exposes; infrastructure and operator-managed fields stay Owner-only.

This matches today's posture exactly: an authenticated user calling `/openai/models` does not see upstreams or endpoints, and the new `GET /v1/models/public/{name}` Public projection MUST NOT regress that. Implementations Phase 2–3 may realize the allowlist via dedicated public DTOs (continuing the `ModelData` / `ApplicationData` pattern) or via `@JsonView(Public)` annotations on a curated subset of entity fields — both produce the same observable surface. The fail-closed `DEFAULT_VIEW_INCLUSION = false` setting ensures a forgotten annotation makes a field invisible everywhere rather than silently Public. When new fields are added to an entity, the default is Owner; promoting a field to Public requires an explicit allowlist decision recorded in this document.
...
| `ResourceAuthSettings` | `codeVerifier` | ❌ | **No** — plain `String` field, serialized verbatim by Jackson in `ToolSetService.putToolSet()`. | PKCE verifier. Plaintext in blob today. Encrypted in Phase 3 by extending `ToolSetService.putToolSet()` to invoke the existing `ResourceAuthSettingsEncryptionService` on this field — the same path that already encrypts `clientSecret`. The `@EncryptedField` / `SecretFieldProcessor` route does **not** fire for toolsets (toolsets are not routed through `MergedConfigStore` per §6 / §8 — the dual-mapper write path doesn't apply), so reusing the bespoke service is the only path that actually executes on the toolset write. **Lazy migration of legacy plaintext blobs.** Existing toolset blobs already in production carry `codeVerifier` as plaintext (no `Base64`-shaped ciphertext), so a naive `decryptValue()` invocation on read would throw `IllegalArgumentException` from `Base64.getDecoder().decode()`. Phase 3 must therefore extend `ResourceAuthSettingsEncryptionService.processFields()` (which today only handles `clientSecret` — see `ResourceAuthSettingsEncryptionService.processFields()` body) to also process `codeVerifier`, and the read path (`decryptValueWithLegacyFallback()`) must classify the stored value three ways: (1) **Base64 decode fails** (`IllegalArgumentException` from the decoder) — not our ciphertext, return the value verbatim as legacy plaintext; (2) **decode succeeds but the decoded length is below the new-format minimum** (`CredentialEncryptionService.minEncryptedLength()` = IV bytes + GCM-tag bytes, 28 with defaults) — too short to be a ciphertext from this service, return verbatim with no decrypt attempt (deterministic); (3) **decode and length checks pass but AES decrypt throws** — a residual ambiguous branch (a long valid-Base64 plaintext that happens to clear the length floor), log at WARN and return the value verbatim, re-encrypting it on the next write. The fallback never rethrows on a `codeVerifier` decrypt failure. This mirrors the legacy-plaintext handling pattern used by `SecretFieldProcessor` for the `ENC[`/`${SECRET:`/plaintext branches; the lazy re-encrypt on next write keeps the migration job-free. |
...
**Blob format — encrypted field marker:** `"ENC[AES256-GCM,data:base64...,iv:base64...,tag:base64...,aad:...]"` prefix makes encrypted values self-identifying. Non-encrypted values (from config files or legacy) have no prefix and pass through as-is. The encrypt path does not trust the prefix alone: it also validates envelope structure (decodable Base64, decoded length ≥ the AES-GCM minimum of IV 12 + tag 16 = 28 bytes) before treating an `ENC[...]`-shaped value as already-encrypted; an `ENC[`-shaped but structurally invalid value is encrypted as plaintext rather than preserved. The read (decrypt) path stays prefix-gated and fails loudly on a malformed `ENC[...]` payload.
...
`PUT` is upsert (see [`03-api-reference.md`](03-api-reference.md) §3) — the controller distinguishes "create" from "update" by reading the pre-state inside the same `LockService` scope as the write (the same pre-read the preserve-on-omit logic needs). Slice U.4 (2026-05-25) retired the `"***"` mask sentinel; secret fields drop from GET responses via `@JsonProperty(access = WRITE_ONLY)` (Jackson default behavior with no annotation introspector override on the response `ObjectMapper`). The secret-field policy splits along pre-state, not along HTTP method:

| Operation | Pre-state | Secret field behavior |
|-----------|-----------|-----------------------|
| `GET` | (any) | Field dropped from the response (`@JsonProperty(WRITE_ONLY)`). No reveal flow. |
| `PUT` (creating: pre-state absent) | absent | Field absent/null → store as null (no secret set on create). Field present → encrypt and store. A literal `"***"` is treated as a real string value (no longer a special sentinel) and re-encrypted as-is. |
| `PUT` (updating: pre-state present) | present | Field absent/null → preserve existing encrypted value (preserve-on-omit). Field present → encrypt and store. A literal `"***"` is a real value here too — clients that round-trip a GET response and re-PUT it will omit the field naturally (it's not in the GET body), so preserve-on-omit fires; clients that explicitly send `"***"` overwrite the prior secret with that literal string. |
...
2. For each `@EncryptedField` field on the entity, if the request body has the field **absent** or **`null`**, the controller substitutes the corresponding ciphertext value from the existing blob.
...
Net effect: a client GET-merge-PUT round-trip is safe because GET responses simply omit the secret field (WRITE_ONLY); the round-tripped body re-arrives without that field and preserve-on-omit fires. Phase 2 implements this in the per-entity `PUT` controller for every entity type whose data class carries `@EncryptedField` annotations (`Model.upstreams[].key`, `Model.upstreams[].extraData`, `Key.key`); toolset writes preserve `clientSecret` and `codeVerifier` through the existing `ResourceAuthSettingsEncryptionService` path, which has equivalent preserve-on-omit semantics handled inside `ToolSetService.putToolSet()`. For array-element secrets (`Model.upstreams[]`), preservation matches request elements to stored elements by their `endpoint` value (with a stable relative-order tiebreak for duplicate endpoints and an index fallback for non-keyed arrays), not by array position — so reordering or inserting upstreams keeps each upstream's prior ciphertext bound to its own endpoint rather than to whatever element now occupies the same index.
...
### 2.6 Optional secret read access *(deferred — Slice U.4)*

Slice U.4 (2026-05-25) retired this feature per DIAL-team review-call decision. The design is preserved below for re-introduction in a later phase.

> **Design draft, not in MVP.** Operators with a separate `security-admin` role could retrieve plaintext secrets via `GET /v1/{type}/{bucket}/{name}?reveal_secrets=true`. The `security-admin` tier was distinct from admin (operator-vetted, separately configured via bootstrap settings) and was **stronger** than the Owner view in §1.5 — it granted plaintext-secret reveal in addition to the Owner-view fields. Reuses of the tier (gating the file-config `keys` endpoint, plaintext reveal on per-entity GET, …) all sat on the same gate. The MVP ships with no plaintext-read path on the API; operators inspect secret values via the underlying KMS / blob layer.
...
- **Existing bare-Base64 ciphertext on `ResourceAuthSettings.clientSecret` and (Phase 3+) `ResourceAuthSettings.codeVerifier`** — produced by `ResourceAuthSettingsEncryptionService.encryptValue()` (no `ENC[` prefix, just `Base64.getEncoder().encodeToString(...)`), stored on toolset blobs. `clientSecret` has lived on this path since before this proposal; Phase 3 extends the same service to also encrypt `codeVerifier` since toolsets do not flow through `MergedConfigStore` and `SecretFieldProcessor` would not fire on the toolset write. Both fields are decrypted by the existing `ResourceAuthSettingsEncryptionService.decryptValue()`. Toolset reads must keep using that service for these two fields rather than routing through `SecretFieldProcessor`, otherwise the bare-Base64 payloads would be misinterpreted as plaintext (no `ENC[` prefix matches) and silently fail downstream OAuth / PKCE flows.
  ...
  **Legacy-plaintext handling for `codeVerifier` (Phase 3 read-path invariant).** Existing toolset blobs in production carry `codeVerifier` as plaintext today; the Phase 3 read path (`decryptValueWithLegacyFallback()`) must therefore distinguish "legacy plaintext" from "new bare-Base64 ciphertext" before trusting `decryptValue()`. The decryption invariant for `codeVerifier` is a **three-way classification**:
  1. **Base64 decode fails** — `Base64.getDecoder().decode()` throws `IllegalArgumentException` (e.g. a PKCE verifier with URL-safe `-`/`_` characters). The value is not our ciphertext; return it verbatim as legacy plaintext.
  2. **Decode succeeds, decoded length below the new-format minimum** — `CredentialEncryptionService.minEncryptedLength()` (IV bytes + GCM-tag bytes = 28 with defaults) is the floor below which a byte sequence cannot be a ciphertext produced by this service. Return the value verbatim with no decrypt attempt. This branch is deterministic — no AES call, no exception.
  3. **Decode and length checks pass, but AES decrypt throws** — the residual ambiguous branch: a long valid-Base64 plaintext that happens to clear the length floor. Log at WARN (`codeVerifier failed AES decrypt; treating as legacy plaintext, will re-encrypt on next write; … the blob may be corrupt`) and return the value verbatim. The fallback **never rethrows** here, so a genuinely corrupt blob is degraded-but-readable rather than fatal; the next toolset write re-encrypts through the encrypted path.

  This mirrors `SecretFieldProcessor`'s prefix-based fallthrough to a "plaintext" branch and keeps the migration lazy — no separate one-shot blob-rewrite job. `clientSecret` does not need this guard (its `decryptValue()` path hard-fails on decrypt error, which is correct — `clientSecret` has no legacy-plaintext history); only `codeVerifier` carries pre-Phase-3 plaintext payloads.

  **Why bare-Base64 is safe under the dual-path invariant.** `SecretFieldProcessor` distinguishes its three formats (plaintext / `ENC[...]` / `${SECRET:...}`) primarily by **prefix** — on the encrypt path it additionally validates the `ENC[...]` envelope's structure (valid Base64, decoded length ≥ the AES-GCM minimum) before trusting it as already-encrypted, while the read path stays prefix-gated and fails loudly; in neither case can it tell, from the value content alone, that a bare-Base64 string is ciphertext rather than a literal plaintext key. The "stay on bespoke path" invariant for `clientSecret` is therefore not a runtime check on value content; it is enforced by the **absence of `@EncryptedField` on the field** (§2.4 negative annotation rule + reflective unit test). Field identity, not value shape, is what keeps the two encryption paths from colliding.
