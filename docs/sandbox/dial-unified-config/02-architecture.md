...
| Distributed locking (LockService) | ✅ Production | Reuse as-is via `LockService.underBucketLocks(MergedConfigStore.ADMIN_BUCKET_LOCATIONS, …)` — serializes admin writes AND replica read paths (rebuild / replica-event / reload) on the same `public/`+`platform/` bucket-lock pair (see §4.4) |
...
Every admin write path — per-entity `PUT`/`DELETE` on `/v1/{type}/{bucket}/{name}` for the admin scope (both `public/` admin-managed types and `platform/`) and bulk `POST /v1/admin/apply` — acquires **two** cluster-wide bucket locks (`public/` + `platform/`) via `LockService.underBucketLocks(MergedConfigStore.ADMIN_BUCKET_LOCATIONS, ...)` around its write phase. Concurrent admin writes on different DIAL Core pods are therefore strictly serialized at the entity-set level, not just per-resource-key.

**Replica-read paths share the same locks (locked 2026-05-26).** A writer pod's apply runs `resourceService.putResource(...)` once per entity in a multi-blob batch, then performs the in-memory `Config` swap *after* every blob is written. During that interval (`T_write_first_blob` → `T_inmemory_swap`) the blob store carries partial-batch state. Any other pod whose `MergedConfigStore.rebuild()`, `applyReplicaEvent`, `reload()`, or boot-time `init()` runs a blob scan in this interval would seed a torn `Config`. The fix: those replica-read paths acquire the same `ADMIN_BUCKET_LOCATIONS` lock set before scanning. The lock-holding writer pod thus owns the read horizon for both buckets across the cluster until the volatile swap completes.

**Why both bucket locks (`public/` + `platform/`) rather than a single global key.** Admin entities span both buckets (models/schemas live in `public/`; interceptors/roles/keys/routes in `platform/`), and a single apply batch may write to both. Acquiring the two bucket locks in canonical sorted order — `LockService.underBucketLocks` sorts internally — prevents the cross-bucket interleaving where pod A's writes to `platform/` and pod B's writes to `public/` could otherwise commit out of order relative to a cross-bucket invariant (e.g., a `MODEL` in `public/` referencing an `INTERCEPTOR` in `platform/`). Bucket-keyed locks also slot into the existing per-bucket `ResourceService` lock fabric without adding new lock vocabulary. Admin writes are rare on real environments (dozens/day at most — see §6 "Why this approach is correct"), so paying the per-rebuild Redis round-trip on the replica read path is acceptable in exchange for a correct cluster-wide read horizon.

**Lock-ordering invariant.** Callers acquire `ADMIN_BUCKET_LOCATIONS` in canonical sorted order (`underBucketLocks` enforces this), **before** any per-resource `ResourceService` lock. Non-admin paths (publications, code-interpreter, user resource CRUD) never acquire `ADMIN_BUCKET_LOCATIONS`, so the consistent ordering precludes deadlock with the existing per-resource lock fabric. `LockService.lock` is **not reentrant at the Redis layer** — call sites must not nest `underBucketLocks` invocations within each other on the same thread. Within `MergedConfigStore`, `apply*` methods are entered *while the caller already holds* the lock (controllers, `applyReplicaEvent`, the timer-fire dispatch); they themselves acquire only the inner intra-pod `rebuildLock` and never re-acquire `ADMIN_BUCKET_LOCATIONS`.

**Lock layering inside `MergedConfigStore`.**

| Outer (distributed, cross-pod) | Inner (intra-pod) | Acquired by |
|---|---|---|
| `ADMIN_BUCKET_LOCATIONS` via `underBucketLocks` | `rebuildLock` (`ReentrantLock`) | `init`, `reload`, `rebuildNow`, debounced timer-fire, `applyReplicaEvent` dispatch, write controllers around `apply*` |
| — | `rebuildLock` only | `applyEntityWrite`, `applyEntityDelete`, `applyBatch`, `applySettingsWrite`, `applySettingsDelete` (their callers hold the outer lock) |

The inner `rebuildLock` is retained as defensive same-pod serialization between the apply-time and timer-fire paths; the outer distributed lock provides cross-pod serialization plus the cluster-wide read horizon. Removing `rebuildLock` would require contract-changing every public `apply*` to "must be called under the outer lock" — a future cleanup, tracked under simplicity opportunities, not load-bearing for the MVP.

**Scope.** The outer lock brackets the entire write phase (including the `MergedConfigStore.applyBatch` partial-update swap) *and* every replica-side blob scan. The lock-holder pod thus commits its writes AND its volatile-`Config` swap before any other pod's rebuild, replica-event apply, or reload can scan blob storage. Peer pods are notified through the existing pub/sub fan-out (§4.6/§Phase 1.5); the pub/sub event handler now also acquires the outer lock before fetching the new entity, so the fast-path apply sees the writer's fully-committed state.

**Forward-compatibility with MT.** Under future multi-tenancy, each top-level scope (`platform/`, `tenants/{id}`, `teams/{id}`, …) gets its own bucket-lock entry in the `underBucketLocks` set — admin writes that span scopes acquire the union of their scopes' locks in sorted order, preserving deadlock-freedom. No design change is required when MT scopes are introduced.
...
- Blob writes are serialized per-entity by `LockService`'s distributed lock (see §2 — "Distributed locking via `LockService`"); admin writes and replica-side rebuilds are additionally serialized cluster-wide by the `ADMIN_BUCKET_LOCATIONS` bucket locks (§4.4), so even cross-entity admin batches commit consistently across replicas and no rebuild scans partial-batch state.
