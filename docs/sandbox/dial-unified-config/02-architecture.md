...
**Per-entity GET response shape (Owner view example — Public view omits `source` and `validationWarnings`)** — also in [`03-api-reference.md`](03-api-reference.md) §2. Field projection follows the Public/Owner views in [`04-security-and-audit.md`](04-security-and-audit.md) §1.5:
...
Every admin write path — per-entity `PUT`/`DELETE` on `/v1/{type}/{bucket}/{name}` for the admin scope (both `public/` admin-managed types and `platform/`) and bulk `POST /v1/admin/apply` — acquires a single cluster-wide lock (`AdminWriteLockService`, backed by `LockService.lock("admin-writes")`) around its write phase. Concurrent admin writes on different DIAL Core pods are therefore strictly serialized at the entity-set level, not just per-resource-key.
