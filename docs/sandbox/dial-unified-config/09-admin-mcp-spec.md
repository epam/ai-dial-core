...
| `dial_admin_get_runtime_config(env, type?)` | `GET /v1/admin/export` | Full merged runtime config or single type. Includes both file-sourced and API-managed entries — the canonical answer to "what entities does this environment actually have" because the folder-metadata listing surface is blob-only. |
