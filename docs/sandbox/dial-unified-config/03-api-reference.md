...
**Entity name validation.** The Configuration API validates entity names on write — returns HTTP 400 for names that don't match `^[A-Za-z0-9._%:-]+$` per path segment. Validation runs on the **URL-decoded** name (so a literal `%` must be sent as `%25`); see [`02-architecture.md`](02-architecture.md) §4 for the full percent-encoding and extendability note. `ConfigPostProcessor` retains its stripping of invalid toolset names as a safety net for file-sourced entities.
...
**Array element pairing.** For secret-bearing array fields (`Model.upstreams`), preserve-on-omit pairs each request element with a stored element to know which prior ciphertext to carry forward. Pairing is deterministic: (1) an element carrying an `endpoint` matches the first unconsumed stored element with the same `endpoint`; (2) an element **without** an `endpoint` falls back to strict same-index pairing against the stored array; (3) when no pair is found — endpoint unmatched, the index slot already consumed by an earlier match, or the index out of bounds — nothing is preserved and the request value (or `null`) wins. Pairing never throws. **Caveat:** because an endpoint match can consume the slot an endpoint-less element would otherwise take by index, mixing endpoint-keyed and endpoint-less elements in one array can silently drop a stored secret; clients should supply secrets explicitly for unkeyed elements or avoid the mix.
...
**Source is blob-only.** The metadata listing enumerates **blob-managed entries only** (via `ResourceService.getMetadata(descriptor)` — the same primitive every existing Resource API listing uses). File-sourced entries (`aidial.config.json`) are **not surfaced** through the metadata listing — they live in `Config` only, have no `ResourceDescriptor`, no `ETag`, no `createdAt`. This is full alignment with the existing Resource API which has no file-source notion at all. *(A runtime-config snapshot endpoint `GET /v1/admin/export` was originally specified for the "what entities are effectively configured right now?" view; deferred — see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md). Until it ships, operators with file-sourced entries in flight consult `aidial.config.json` directly off the deployment.)*
...
Validation is **strict** — an unknown `kind` value on `POST /v1/admin/apply` is reported as a per-entity `FAILED` result: under `precheck: true` (the default) the batch is rejected with `422 Unprocessable Entity` (nothing applied, valid siblings reported `skipped`); under `precheck: false` it appears as a per-entity `FAILED` inside the `200 OK` batch. `400 Bad Request` remains reserved for envelope-level malformations (non-object body, missing `manifests`, non-string `kind`, `Bundle`). The CLI rejects unknown `kind` at parse time before sending. Overlay variants (`*Overlay`) are CLI-only and never appear in the apply payload sent to the server — the CLI resolves base + overlay into a `kind: Model` / `kind: Role` / etc. before submission. The `Bundle` kind is also CLI-only — bundles expand into their constituent entity manifests client-side per [`05-cli-design.md`](05-cli-design.md) §5.3, so the server never sees `kind: Bundle`.
...
> **Phase gate — bulk write/validate ships in [Phase 4](07-migration-and-rollout.md#phase-4-declarative-mode--environment-promotion).** Phases 2 and 3 deliver per-entity `PUT` / `DELETE` only. The bulk-write endpoint `POST /v1/admin/apply` and the `dial-cli apply` / `diff` commands ship in Phase 4. `POST /v1/admin/validate` ships in two stages: a model-scoped validate in Phase 2 (covers the same single-entity types Phase 2 makes writable), and the full multi-entity / batch-aware validate in Phase 4 alongside `apply`. The read-only snapshot `GET /v1/admin/export` (and `dial-cli export`) is **deferred from MVP** (see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md)); design preserved. Until Phase 4, operators issue per-entity calls or use the [§6 migration workflow](#6-validation) to seed batches.
