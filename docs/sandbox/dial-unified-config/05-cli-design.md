...
**Wire shape — full alignment with the existing Resource API.** Every CLI write verb maps to `PUT` on the per-entity URL — the same way operators write against `applications`, `toolsets`, `conversations`, `prompts`, and `files` today. The CLI verbs are client-side ergonomic differentiators on top of one wire shape: `add` issues `PUT … If-None-Match: *` (create-only — exits `5` on `412` when the entity already exists); `update <name> --if-match <etag>` issues `PUT … If-Match: <etag>` (CAS update — exits `6` on `412` mismatch); a bare `update <name>` issues an unconditional `PUT` (last-write-wins). The server side has no `POST` at the single-entity surface (returns `405` per [`03-api-reference.md`](03-api-reference.md) §3). Bulk upsert (create-or-update across many entities by desired-state) lives on `dial-cli apply -f` (which the server processes via `POST /v1/admin/apply`).

**Exit codes.** The CLI's complete exit-code contract for CI/CD pipelines lives in [`06-cli-user-guide.md`](06-cli-user-guide.md) §2.8 — `0` (success / nothing to apply), `1` (partial-batch runtime failure), `2` (validation failure), `3` (auth failure), `4` (404 — entity not found), `5` (412 — `If-None-Match: *` rejected because entity exists), `6` (412 — `If-Match: <etag>` mismatch). Per-resource-type commands and `apply` share the same code mapping. The `5`/`6` split reflects which conditional header the CLI sent — both map to the same `412` on the wire.
...
| `delete <name> [--if-match <etag>]` | `DELETE` | Delete — exits `4` on `404` if entity does not exist; `--if-match <etag>` adds the CAS guard (exit `6` on `412`). |

> **`settings update` shares the wire shape.** `dial-cli settings update` maps to `PUT /v1/settings/platform/global` — same `PUT`-upsert as every other admin-config type. The singleton's GET projection always has a value (`"api"` / `"file"` / `"default"`), so the exit-`4` mapping for missing entities never applies here. All other exit codes (`0`, `2`, `3`, `5` if the operator passes `--if-none-match` on a singleton that already has an API blob, `6` on `--if-match` mismatch) apply unchanged.
>
...
| `promote --from <env> --to <env> --name <n> [--template <t>\|auto] [--param k=v]` | `GET` (source) + `POST /v1/admin/apply` (target) | Promote between environments. The CLI fetches the source entity, transforms env-specific fields, and submits a single-entity manifest to `POST /v1/admin/apply` — the canonical batch-upsert path — which is preferred over a client-side per-entity `PUT` because the apply endpoint also runs precheck-aware validation across the whole batch when multiple entities are promoted together. |
...
**Update ergonomics without PATCH.** Phase 2–3 API exposes `GET`, `PUT` (upsert), and `DELETE` only — see [`03-api-reference.md`](03-api-reference.md) §1. The CLI provides field-level update UX via `--set` flags:
...
8. **Apply** to target environment via `POST /v1/admin/apply` with a single-entity manifest. This is the canonical batch-upsert path (see [`03-api-reference.md`](03-api-reference.md) §1, §7) and is preferred over a per-entity `PUT` because the apply endpoint runs batch-aware validation (precheck) and applies the entity under the same cluster-wide admin-write lock as cross-entity batches.
