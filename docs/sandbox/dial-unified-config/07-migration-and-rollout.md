...
- *(deferred — see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md))* Implement read-only `GET /v1/admin/export` (snapshot of the current in-memory `Config` for inspection / bootstrap-export workflows). Deferred from MVP at core-team request 2026-05-20; design preserved. The bulk write surface — `POST /v1/admin/apply` / `validate` and the equivalent `dial-cli apply` / `validate` commands — ships in **Phase 4**, see [`03-api-reference.md`](03-api-reference.md) §7.
- These read directly from the in-memory `volatile Config` ref — zero storage changes.
- Protected by `access.admin.rules` via `ConfigAuthorizationService`. **Fail-closed:** the Configuration/Admin API gates on `AccessService.hasExplicitAdminAccess`, which denies all config/admin access (platform reads+writes, public writes, every `/v1/admin/*`) when `access.admin.rules` is empty or unconfigured — unlike the legacy `RuleMatcher` empty→allow-all default. **Operator action:** any operator who wants to use the config/admin API must configure at least one admin rule under `access.admin.rules` (e.g. `{ "function": "CONTAIN", "source": "roles", "targets": ["admin"] }`); an empty/absent block means the entire config/admin surface returns `403`. Public reads stay open and legacy (non-config) authorization paths are unaffected. See [`04-security-and-audit.md`](04-security-and-audit.md) §1.3.
...
- Implement `PUT /v1/models/public/{name}` — upsert with `If-None-Match: *` (create-only gate: `412` if entity already exists) and `If-Match: <etag>` (CAS update guard: `412` on mismatch); unconditional write when neither header is present. `POST` on that path returns `405` with `Allow: GET, PUT, DELETE`. `DELETE /v1/models/public/{name}`. See `03-api-reference.md` §1 and §3. Bucket-aware authz via `ConfigAuthorizationService` per [`04-security-and-audit.md`](04-security-and-audit.md) §1.2.
...
- Implement `model add`, `model update`, `model delete` with templates. Wire mapping per U.0: `model add` → `PUT … If-None-Match: *` (412 if exists); `model update` → `PUT` (unconditional upsert; `--if-match <etag>` adds CAS guard, 412 on mismatch); `model delete` → `DELETE` (404 if missing, `--if-match` adds CAS guard). See `05-cli-design.md` §1 and `06-cli-user-guide.md` §2.4.
...
- *(deferred — see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md))* Implement `dial-cli export --env <env>` → YAML manifests. Deferred alongside the underlying `GET /v1/admin/export` endpoint; design preserved.
...
- Exit code `0` if all succeeded, `1` if any failed in the batch. Full CLI exit-code contract — `0` (success / nothing to apply), `1` (partial-batch failure), `2` (validation), `3` (auth), `4` (404), `5` (412 on `If-None-Match: *` — entity already exists; use update instead), `6` (412 on `If-Match` mismatch — re-read and retry) — is in [`06-cli-user-guide.md`](06-cli-user-guide.md) §2.8.
...
- **Replace CRUD controllers with API pass-through.** Every `/api/v1/*` endpoint in Admin Backend that today writes to its DB is rewritten to call the corresponding Configuration API endpoint on DIAL Core — per-entity CRUD goes to `/v1/{type}/{bucket}/{name}`, bulk/declarative writes go to `/v1/admin/apply`. Exports historically came from `GET /v1/admin/export`; that endpoint is deferred (see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md)) — Admin Backend's export surface is gated on it shipping. Admin Backend stays in the call path for: (a) session / CSRF handling tied to the Admin UI, (b) OIDC / Basic Auth login that the UI uses, (c) any UI-specific aggregation or denormalization that doesn't fit the generic API surface.
...
- **Migration of existing data.** For environments already running Admin Backend: on first boot against the new Configuration API, a one-shot import job reads the Admin Backend DB and issues equivalent per-entity writes (`PUT /v1/{type}/{bucket}/{name}` with `If-None-Match: *` for create-only semantics) — or one bulk `POST /v1/admin/apply` — to seed DIAL Core, then the DB schema is retired. This is a one-way migration; once the import completes the DB is no longer consulted for config.
...
- **Multi-destination export (Vault / AWS SM / GCP SM / Azure Key Vault / K8s ConfigMap) is deprecated and removed in Phase 5.** The `FileConfigStore` and config-file approach are unaffected and continue to work (Phase 6 optional deprecation). The exporters' role was specifically to feed `FileConfigStore` consumers (write config to a file / secret store → mount into the DIAL Core pod → `FileConfigStore` polls it). The intended migration path is scheduled `GET /v1/admin/export` writes — a customer-owned script against the new API, not an Admin Backend feature; that endpoint is deferred from MVP (see [IMPLEMENTATION.md §5.5 Defer.1](IMPLEMENTATION.md)) and Phase 5 cannot land until it ships. Customers with external backup / DR / cross-tool workflows that were incidentally riding on these exporters migrate the same way once export is available.
