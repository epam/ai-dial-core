...
* `configurationEndpoint`: A URL to fetch dynamic model-specific settings (e.g. per-tenant max tokens, allowed parameters). Use this to drive runtime overrides from a remote config store. Use to request model's configuration parameters as JSON schema. Exposed by DIAL Core as `<deployment name>/configuration`.
